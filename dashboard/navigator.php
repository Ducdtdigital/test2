...
elseif ($_REQUEST['act'] == 'add') {
    ...
    elseif ($_REQUEST['step'] == 2) {
        ...
        // Thêm dòng này để lấy cat_id từ request
        $cat_id = isset($_REQUEST['menulistcat']) ? intval($_REQUEST['menulistcat']) : 0;
        ...
        if($item_ifshow == 1 && $item_type == 'middle') {
            ...
            if($arr) {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,title,ctype,cid,ifshow,vieworder,opennew,url,type,class,cat_id) VALUES('$item_name','$title',".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$class', '$cat_id')";
            }
        }
        if(empty($sql)) {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,title,ifshow,vieworder,opennew,url,type,class,cat_id) VALUES('$item_name','$title','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$class', '$cat_id')";
        }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_REQUEST['step']))
    {
        ...
        $rt['item_url'] = $row['url'];
        $rt['item_cat_id'] = $row['cat_id']; // Thêm dòng này để lấy cat_id từ dữ liệu
       // var_dump($row['cat_id']);die;
        $rt['class'] = $row['class'];
        $rt['title'] = $row['title'];
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        $item_name = $_REQUEST['item_name'];
        $item_url = $_REQUEST['item_url'];
        $item_cat_id = (isset($_REQUEST['menulistcat']) && $_REQUEST['menulistcat'] != '-') ? intval($_REQUEST['menulistcat']) : 0; 
        $item_ifshow = $_REQUEST['item_ifshow'];
        $item_opennew = $_REQUEST['item_opennew'];
        $item_type = $_REQUEST['item_type'];
        /** add by Nobj */
        ...
        $navs = array(
            ...
            'type'=> $item_type,
            'class'=> $class,
            'cat_id' => $item_cat_id // Thêm dòng này để cập nhật cat_id trong cơ sở dữ liệu
         );

        $db->autoExecute($ecs->table('nav'), $navs, 'UPDATE', "id='$id'");
...
function get_sysnav()
{
    global $_LANG;
    $sysmain = array(
        array($_LANG['view_cart'],'gio-hang'),
        );
    $sysmain[] = array('-','-');
...
function get_sysnavmenu()
{
    ...
    $sysmain[] = array('-','-');
    $catlist = array_merge(cat_list_menu(false), array('-')); 

    $new_sysmain = []; // Tạo một mảng mới để lưu trữ dữ liệu
    ...
    foreach($catlist as $key => $val)
    {
        if(is_array($val))
        {
            ...
            // Sử dụng cat_id làm key cho mảng mới
            $new_sysmain[$val['cat_id']] = array($val['cat_name'], $val['url'], $val['view_name']);
        }
    }
    return $new_sysmain; // Trả về mảng mới
}
