...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $cat_name = !empty($_POST['cat_name']) ? filter_var($_POST['cat_name'], FILTER_SANITIZE_STRING) : '';
    $meta_title = !empty($_POST['meta_title']) ? filter_var($_POST['meta_title'], FILTER_SANITIZE_STRING) : '';
    $is_hide = !empty($_POST['is_hide']) ? $_POST['is_hide'] : 0;

    $sql = "INSERT INTO ".$ecs->table('article_cat')."(cat_name, cat_type, meta_title, cat_desc,keywords, parent_id, sort_order, show_in_nav, is_hide)
           VALUES ('$cat_name', '$cat_type', '$meta_title', '$_POST[cat_desc]','$_POST[keywords]', '$_POST[parent_id]', '$_POST[sort_order]', '$_POST[show_in_nav]', '$is_hide')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT cat_id, cat_name, meta_title, cat_type, cat_desc, show_in_nav, is_hide, keywords, parent_id,sort_order FROM ".
    ...
    $slug = get_slug($_REQUEST['id'], 'article_cat');
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($exc->edit("cat_name = '$cat_name', meta_title = '$meta_title', cat_desc ='$_POST[cat_desc]', keywords='$_POST[keywords]',parent_id = '$_POST[parent_id]', cat_type='$cat_type', sort_order='$_POST[sort_order]', show_in_nav = '$_POST[show_in_nav]', is_hide = '$is_hide'",  $_POST['id']))
    {
...
//-- 切换是否显示在导航栏
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'toggle_is_hide')
{
    check_authz_json('cat_manage');

    $id = intval($_POST['id']);
    $val = intval($_POST['val']);
    if (cat_update($id, array('is_hide' => $val)) != false)
    {
        clear_cache_files();
