...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('article')."(title, article_thumb, article_sthumb, cat_id, article_type, is_open, is_hot, author, ".
                "author_email, keywords, content, add_time, post_time, file_url, open_type, link, description) ".
            "VALUES ('$_POST[title]', '$thumb[thumb]', '$thumb[sthumb]', '$_POST[article_cat]', '$_POST[article_type]', '$_POST[is_open]', '$_POST[is_hot]', ".
                "'$_POST[author]', '$_POST[author_email]', '$_POST[keywords]', '$_POST[content]', ".
                "'$add_time', '$post_time', '$file_url', '$open_type', '$_POST[link_url]', '$_POST[description]')";
    $db->query($sql);
    /* 处理关联商品 */
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    /* 创建 html editor */
    create_html_editor('content', $article['content'], 'FCKeditor', 'Normal', '100%', '500');
    /* 取得分类、品牌 */
    $smarty->assign('goods_cat_list', cat_list());
    $smarty->assign('brand_list', get_brand_list());
...
if ($_REQUEST['act'] =='update')
{
    ...
    if ($exc->edit("title='$_POST[title]', {$sql_update} cat_id='$_POST[article_cat]', article_type='$_POST[article_type]', is_open='$_POST[is_open]', post_time='$post_time', author='$_POST[author]', author_email='$_POST[author_email]', keywords ='$_POST[keywords]', file_url ='$file_url', open_type='$open_type', is_hot='$is_hot', content='$_POST[content]', link='$_POST[link_url]', description = '$_POST[description]'", $_POST['id']))
    {
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    @unlink(ROOT_PATH . $rm['article_thumb']);
    @unlink(ROOT_PATH . $rm['article_sthumb']);
...
function upload_thumb(){
        ...
        isset($_FILES['article_thumb']['tmp_name']) &&$_FILES['article_thumb']['tmp_name'] != 'none')
    {
        ...
        $dir = ROOT_PATH .'images/' . date('Ym').'/thumb_article/';
    ...
    return array('thumb' => $thumbnail, 'sthumb' => $small_thumb);
}
function unlink_old_thumb(){
    @unlink(ROOT_PATH. $_POST['old_thumb']);
    @unlink(ROOT_PATH. $_POST['old_sthumb']);
}
?>