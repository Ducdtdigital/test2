...
function login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	if ( $wp_error->has_errors() ) {
		...
		if ( ! empty( $errors ) ) {
			 ...
			 */
			echo '<div id="login_error">' . apply_filters( 'login_errors', "Sai tài khoản hoặc mật khẩu" ) . "</div>\n";
		}
...
switch ( $action ) {
	...
	case 'confirm_admin_email':
		 ...
		 * as the current user is not set, see index-includes/pluggable.php.
		...
		<form class="admin-email-confirm-form" name="admin-email-confirm-form" action="<?php echo esc_url( site_url( 'index-login.php?action=confirm_admin_email', 'login_post' ) ); ?>" method="post">
			<?php
			/**
	...
	case 'retrievepassword':
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_REQUEST['redirect_to'] ) ? $_REQUEST['redirect_to'] : 'index-login.php?checkemail=confirm';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
	...
	case 'rp':
		...
		if ( ! $user || is_wp_error( $user ) ) {
			...
			if ( $user && $user->get_error_code() === 'expired_key' ) {
				wp_redirect( site_url( 'index-login.php?action=lostpassword&error=expiredkey' ) );
			} else {
				wp_redirect( site_url( 'index-login.php?action=lostpassword&error=invalidkey' ) );
			}
		...
		?>
		<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( network_site_url( 'index-login.php?action=resetpass', 'login_post' ) ); ?>" method="post" autocomplete="off">
		...
		<p id="nav">
			...
			if ( get_option( 'users_can_register' ) ) {
				...
				/** This filter is documented in index-includes/general-template.php */
	...
	case 'register':
		...
		if ( ! get_option( 'users_can_register' ) ) {
			wp_redirect( site_url( 'index-login.php?registration=disabled' ) );
			exit;
		}
		...
		if ( $http_post ) {
			...
			if ( ! is_wp_error( $errors ) ) {
				$redirect_to = ! empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : 'index-login.php?checkemail=registered';
				wp_safe_redirect( $redirect_to );
				exit;
			}
		}
		...
		?>
		<form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'index-login.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
			<p>
	...
	case 'checkemail':
		...
		/** This action is documented in index-login.php */
	...
	default:
		...
		if ( isset( $_REQUEST['redirect_to'] ) ) {
			...
			if ( $secure_cookie && false !== strpos( $redirect_to, 'indexapi' ) ) {
		...
		if ( ! is_wp_error( $user ) && ! $reauth ) {
			...
			if ( is_a( $user, 'WP_User' ) && $user->exists() && $user->has_cap( 'manage_options' ) ) {
				...
				/** This filter is documented in index-login.php */
				$admin_email_check_interval = (int) apply_filters( 'admin_email_check_interval', 6 * MONTH_IN_SECONDS );
			...
			if ( ( empty( $redirect_to ) || 'indexapi/' === $redirect_to || admin_url() === $redirect_to ) ) {
		...
		if ( $interim_login ) {
			...
			} elseif ( isset( $_GET['redirect_to'] ) && false !== strpos( $_GET['redirect_to'], 'indexapi/authorize-application.php' ) ) {
		...
		<form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'index-login.php', 'login_post' ) ); ?>" method="post">
			<p>
