...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('data_itemObjects', $list['itemObjects']);
...
function get_comment_list()
{
    ...
    $filter['itemObject']     = empty($_REQUEST['itemObject']) ? '' : trim($_REQUEST['itemObject']);
    $filter['itemStatus']     = $_REQUEST['itemStatus'];
    ...
    $where = (!empty($filter['keywords'])) ? " AND (content LIKE '%" . mysql_like_quote($filter['keywords']) . "%' OR telephone LIKE '%" . mysql_like_quote($filter['keywords']) . "%') " : ''; 
    $where .= (!empty($filter['itemObject'])) ? " AND id_value = '" . $filter['itemObject'] . "' " : '';
    $where .= ($filter['itemStatus'] == "0" || $filter['itemStatus'] == "1") ? " AND status = '" . $filter['itemStatus'] . "' " : '';
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $itemObjects[$row[id_value]] = $row['title'];
...
//            $GLOBALS['_LANG']['yes_reply'] : $GLOBALS['_LANG']['no_reply'];
    ...
    $arr = array('item' => $arr, 'itemObjects' => $itemObjects, 'filter' => $filter, 'page_count' => $filter['page_count'], 'record_count' => $filter['record_count']);
