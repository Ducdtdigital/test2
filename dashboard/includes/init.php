...
{
    @mkdir('../temp/caches', 0755, true);
}
if (!file_exists('../temp/compiled/admin'))
{
    @mkdir('../temp/compiled/admin', 0755, true);
}
clearstatcache();
    ...
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order')
{
    ...
    if (!empty($_COOKIE['ECSCP']['admin_id']) && !empty($_COOKIE['ECSCP']['admin_pass']))
    {
        ...
        {
            // 没有找到这个记录
            setcookie($_COOKIE['ECSCP']['admin_id'],   '', 1,$cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
            setcookie($_COOKIE['ECSCP']['admin_pass'], '', 1,$cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
