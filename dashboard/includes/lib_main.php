...
function admin_priv($priv_str, $msg_type = '' , $msg_output = true)
{
    ...
    if (isset($_SESSION['action_list']) && strpos(',' . $_SESSION['action_list'] . ',', ',' . $priv_str . ',') === false)
    {
...
function Ueditor($input_name, $input_value = '', $width = '100%', $height = '350px')
{
    global $smarty;
    $kindeditor="
    <textarea name='$input_name' id='editor_$input_name' style='width:$width;height:$height;'>$input_value</textarea>
    <script>
        var ue = UE.getEditor('editor_".$input_name."');
        UE.getEditor('editor_".$input_name."').focus();
    </script>";
 ...
 * @param string $input_name
 * @param string $input_value
 * @param string $toolbar_type Basic|Full
 * @param string $width
 * @param string $height
 */
function CKeditor($input_name, $input_value = '',$toolbar_type = '', $width = '100%', $height= '350')
{
    global $smarty;
    $toolbar_type = isset($toolbar_type) && $toolbar_type == 'Basic' ? 'Basic' : 'Full';
    $ckeditor = "<textarea name='$input_name' id='editor_$input_name'>$input_value</textarea>
    <script type='text/javascript'>
            CKEDITOR.replace( 'editor_$input_name', {
                width: '$width',
                height: '$height',
                toolbar: '$toolbar_type',
    ";
    /* CKFinder Only for full toolbar */
    if($toolbar_type == 'Full'){
    $ckeditor .= "extraPlugins: 'print,format,font,colorbutton,justify,uploadimage',
                uploadUrl: '../includes/ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Files&responseType=json',
    ...
    $smarty->assign('CKeditor_'.$input_name, $ckeditor);
}
function CKeditorForBrands($input_name, $input_value = '', $toolbar_type = '', $width = '100%', $height= '350')
{
    $toolbar_type = isset($toolbar_type) && $toolbar_type == 'Basic' ? 'Basic' : 'Full';
    $ckeditor = "<textarea name='$input_name' id='editor_$input_name'>$input_value</textarea>
    <script type='text/javascript'>
...
/**
 * Duoc di chuyen tu lib_good qua
 *
 * @access  public
 * @param   integer $goods_id
 * @return  array
 */
function get_goods_articles2($goods_id)
{
    $sql = "SELECT g.goods_id, g.article_id, a.title " .
...
function get_goods_articles3($goods_id)
{
    $sql = "SELECT g.goods_id, g.article_id, a.title " .
            "FROM " .$GLOBALS['ecs']->table('goods_article3') . " AS g, " .
                $GLOBALS['ecs']->table('article') . " AS a " .
            "WHERE g.goods_id = '$goods_id' " .
            "AND g.article_id = a.article_id ";
    if ($goods_id == 0)
...
function get_goods_articles4($goods_id)
{
    $sql = "SELECT g.goods_id, g.article_id, a.title " .
            "FROM " .$GLOBALS['ecs']->table('goods_article4') . " AS g, " .
                $GLOBALS['ecs']->table('article') . " AS a " .
            "WHERE g.goods_id = '$goods_id' " .
            "AND g.article_id = a.article_id ";
    if ($goods_id == 0)
...
function get_slug($id, $module){
    $urlseo= $GLOBALS['db']->getOne("SELECT slug FROM " . $GLOBALS['ecs']->table('slug') . " WHERE id = '$id' AND module='$module'");
    return $urlseo;
}
...
function gen_slug($mod){
    $slug = build_slug($_POST['slug']);
    if ($slug=='')
    {   if($mod == 'goods'){
            $slug = !empty($_POST['title'])? trim($_POST['title']) : trim($_POST['goods_name']);
            $slug = build_slug($url);
        }elseif ($mod == 'category') {
    ...
    if ($check_slug > 0)
    {
        sys_msg($GLOBALS['_LANG']['slug_exists'], 1, array(), false);
        return false;
    }
    return $slug;
}
...
function get_subcategory_goods_types($parent_cat_id) {
    $sql = "SELECT g.cat_id
            FROM " . $GLOBALS['ecs']->table('goods') . " AS g
            INNER JOIN " . $GLOBALS['ecs']->table('category') . " AS c ON g.cat_id = c.cat_id
            WHERE c.parent_id = '$parent_cat_id' OR g.cat_id = '$parent_cat_id'";
    $sub_categories = $GLOBALS['db']->getCol($sql);
    $cat_ids_str = implode(',', $sub_categories);

    $sql = "SELECT DISTINCT g.goods_type
            FROM " . $GLOBALS['ecs']->table('goods') . " AS g
            WHERE g.cat_id IN ($cat_ids_str)";
    $goods_types = $GLOBALS['db']->getCol($sql);

    return $goods_types;
}

function goods_type_list1($cat_id, $parent_cat_id)
{
    // Lấy danh sách các danh mục con của danh mục cha
    $sub_categories = get_sub_categories($parent_cat_id);

    // Lấy danh sách goods_type_id (cat_id) từ bảng goods dựa trên danh mục con
    $sql = "SELECT DISTINCT goods_type FROM " . $GLOBALS['ecs']->table('goods') . " WHERE cat_id IN (" . implode(',', $sub_categories) . ")";
    $goods_type_ids = $GLOBALS['db']->getCol($sql);

    // Truy vấn các thuộc tính của các goods_type_id
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['ecs']->table('goods_type') . ' WHERE cat_id IN (' . implode(',', $goods_type_ids) . ') AND enabled = 1';
    $res = $GLOBALS['db']->query($sql);
    ...
    {
        $lst .= "<option value='$row[cat_id]'";
        $lst .= ($cat_id == $row['cat_id']) ? ' selected="true"' : '';
        $lst .= '>' . htmlspecialchars($row['cat_name']). '</option>';
    }
...
{
    $sub_categories = array($parent_cat_id);

    $sql = "SELECT cat_id FROM " . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$parent_cat_id'";
    $children = $GLOBALS['db']->getCol($sql);

    foreach ($children as $child) {
        $sub_categories = array_merge($sub_categories, get_sub_categories($child));
    }
...
function set_filter($filter, $sql, $param_str = '')
{
    ...
    setcookie('ECSCP[lastfilterfile]', sprintf('%X', crc32($filterfile)), time() + 600,$GLOBALS['cookie_path'], $GLOBALS['cookie_domain'], $GLOBALS['cookie_secure'], $GLOBALS['cookie_http_only']);
    setcookie('ECSCP[lastfilter]',     urlencode(serialize($filter)), time() + 600,$GLOBALS['cookie_path'], $GLOBALS['cookie_domain'], $GLOBALS['cookie_secure'], $GLOBALS['cookie_http_only']);
    setcookie('ECSCP[lastfiltersql]',  base64_encode($sql), time() + 600,$GLOBALS['cookie_path'], $GLOBALS['cookie_domain'], $GLOBALS['cookie_secure'], $GLOBALS['cookie_http_only']);
}
/**
 * 取得上次的过滤条件
 * @param   string  $param_str  参数字符串，由list函数的参数组成
...
function slug_exists($slug_name, $module)
{
    $sql = "SELECT COUNT('id_slug') FROM " .$GLOBALS['ecs']->table('slug').
    " WHERE module = '$module' AND slug = '$slug_name'";
    return ($GLOBALS['db']->getOne($sql) > 0) ? true : false;
}
 ...
 * @return      array
 */
function user_list_info($conditions = '')
{
    $where = '';
    if (!empty($conditions))
    {
    ...
    $sql = "SELECT user_id, user_name, fullname
