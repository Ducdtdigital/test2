 ...
 */
function handle_goods_attr($goods_id, $id_list, $is_spec_list, $value_price_list, $goods_code_list)
{
    $goods_attr_id = array();
    /* 循环处理每个属性 */
    foreach ($id_list AS $key => $id)
    {
        ...
        $goods_code = $goods_code_list[$key]; // Move this line to here
        if ($is_spec == 'false')
        {
        ...
        if (!empty($result_id))
        {
            $sql = "UPDATE " . $GLOBALS['ecs']->table('goods_attr') . "
                SET attr_value = '$value', goods_code = '$goods_code'
                WHERE goods_id = '$goods_id'
                AND attr_id = '$id'
        ...
        {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_attr') . " (goods_id, attr_id, attr_value, attr_price, goods_code) " .
             "VALUES ('$goods_id', '$id', '$value', '$price', '$goods_code')";
        }
 ...
 * @return  void
 */
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls, $filename ='')
{
    /* 是否处理缩略图 */
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            if ($proc_thumb && gd_version() > 0)
            {
                ...
                $newname    = dirname($img_original) . '/' . $filename .'-'.$GLOBALS['image']->random_filename() . substr(basename($img_original), $pos);
            ...
            /* 重新格式化图片名称 */
            $img_original = reformat_image_name('gallery', $goods_id, $img_original, 'source', $filename);
            $img_url = reformat_image_name('gallery', $goods_id, $img_url, 'goods', $filename);
            $thumb_url = reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb', $filename);
            ...
            if ($proc_thumb && !$GLOBALS['_CFG']['retain_original_img'] && !empty($img_original))
            {
                ...
                @unlink(ROOT_PATH.CDN_PATH1.'/' . $img_original);
            }
        }
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price, v.goods_code ".
            "FROM " .$GLOBALS['ecs']->table('attribute'). " AS a ".
            "LEFT JOIN " .$GLOBALS['ecs']->table('goods_attr'). " AS v ".
            "ON v.attr_id = a.attr_id AND v.goods_id = '$goods_id' ".
            "WHERE a.cat_id = " . intval($cat_id) ." OR a.cat_id = 0 ".
            "ORDER BY a.sort_order, a.attr_type, a.attr_id, v.attr_price, v.goods_code, v.goods_attr_id";
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        {
            $html .= '<input name="attr_value_list[]" type="text" value="' .htmlspecialchars($val['attr_value']). '" size="40" /> </br>';
        }
        ...
        if ($val['attr_type'] == 1 || $val['attr_type'] == 2)
        {
            $html .= $GLOBALS['_LANG']['spec_price'].':'.' <input class="masp" type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" /> ';
            $html .= '</br>';
            $html .= "Mã sản phẩm: ".' <input type="text" name="goods_code_list[]" value="' . $val['goods_code'] . '" onblur="checkGoodsCode(this.value,' . $goods_id . ',this)" />';
            $html .= '<span class="goods_code_notice"></span>';
        }
        else
        {
            $html .= ' <input type="hidden" name="attr_price_list[]" value="0" />';
            $html .= ' <input type="hidden" name="goods_code_list[]" value="0" />';
        }
...
/* get danh sách quà tặng liên kết */
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, shop_price, is_real, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
 ...
 *
 */
function reformat_image_name($type, $goods_id, $source_img, $position='', $filename = '')
{
    $aname   = explode(".", basename($source_img));
    $bname = array_shift($aname);
    $rand_name = $filename == '' ? $bname : $filename;
    $fname = '-'.gmtime() . sprintf("%03d", mt_rand(1,999));
    $img_ext = substr($source_img, strrpos($source_img, '.'));
    /**
     * Chuyển hình vào cdn1
     * DataSQL thì giữ lại url cũ
     */
    $dir = CDN_PATH1.'/images';
    $img_dir = 'images';
    ...
    {
        case 'goods':
            $img_name = $rand_name.'-G'.$goods_id.$fname;
            break;
        case 'goods_thumb':
            $img_name = $rand_name.'-thumb-G'.$goods_id.$fname ;
            break;
        case 'gallery':
            $img_name = $rand_name.'-P' . $goods_id.$fname ;
            break;
        case 'gallery_thumb':
            $img_name = $rand_name.'-thumb-P'.$goods_id.$fname ;
            break;
    }
    if ($position == 'source')
    {
        ...
        {
            return $img_dir.'/'.$sub_dir.'/source_img/'.$img_name.$img_ext;
        }
    }
    elseif ($position == 'thumb')
    {
        ...
        {
            return $img_dir.'/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext;
        }
    }
    else
    {
        ...
        {
            return $img_dir.'/'.$sub_dir.'/goods_img/'.$img_name.$img_ext;
        }
    }
    return false;
}
