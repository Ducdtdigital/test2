...
elseif ($_REQUEST['act'] =='insert')
{
    ...
    $info = array('package_price'=>$_POST['package_price']);
     /** @var U [description] */
    //$package_img = $image->upload_image($_FILES['package_img'], CDN_PATH.'/package_img', $slug);
    //$package_img = str_replace(CDN_PATH.'/', '', $package_img);
...
elseif ($_REQUEST['act'] =='update')
{
    ...
    {
        $package_img = upthumb();
        @unlink(ROOT_PATH.CDN_PATH.'/'.$_POST['old_package_img']);
        $record = array('act_thumb' => $package_img, 'act_name' => $_POST['package_name'],'start_time' => $_POST['start_time'], 'end_time' => $_POST['end_time'],
	...
	/*if($package_img){
		$record = array('slug'=> $slug, 'act_thumb' => $package_img, 'act_name' => $_POST['package_name'],'start_time' => $_POST['start_time'], 'end_time' => $_POST['end_time'],
                    'act_desc' => $_POST['desc'], 'ext_info'=>serialize($info));
	}else{
		$record = array('slug'=> $slug, 'act_name' => $_POST['package_name'], 'start_time' => $_POST['start_time'], 'end_time' => $_POST['end_time'],
                    'act_desc' => $_POST['desc'], 'ext_info'=>serialize($info));
...
function upthumb(){
    if (isset($_FILES['package_img']) && $_FILES['package_img']['tmp_name'] != '' &&
        isset($_FILES['package_img']['tmp_name']) &&$_FILES['package_img']['tmp_name'] != 'none')
    {
        if (!check_type_allow($_FILES['package_img']['name'], $GLOBALS['allow_image_types']))
        {
            sys_msg($GLOBALS['_LANG']['invalid_file']);
        }

        $filename = build_slug($_POST['package_name']).'-'.time();
        $dir = ROOT_PATH.CDN_PATH.'/package_img/';
        $thumbnail = $GLOBALS['image']->make_thumb($_FILES['package_img']['tmp_name'], 600,315, $dir, '', $filename);
