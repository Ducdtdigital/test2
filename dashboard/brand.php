...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    {
        $_POST['brand_meta_title'] = $_POST['brand_meta_title'];
    }
    if (!empty($_POST['brand_keyword']))
    {
        $_POST['brand_keyword'] = $_POST['brand_keyword'];
    }
    if (!empty($_POST['brand_desc_long']))
    {
        $_POST['brand_desc_long'] = $_POST['brand_desc_long'];
    }
    $logoname = 'logo-'.build_slug($_POST['brand_name']);
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'cdn/brandlogo', $logoname));
    ...
    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order, brand_meta_title,brand_keyword, brand_desc_long) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]', '$img_name', '$is_show', '$_POST[sort_order]', '$_POST[brand_meta_title]', '$_POST[brand_keyword]', '$_POST[brand_desc_long]')";
    $db->query($sql);

    $brand_id = $db->insert_id();
    create_slug($brand_id, 'brand');
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo, is_show, sort_order, brand_meta_title, brand_keyword, brand_desc_long ".
            "FROM " .$ecs->table('brand'). " WHERE brand_id='$_REQUEST[id]'";
    $brand = $db->GetRow($sql);
    CKeditor('brand_desc_long', html_entity_decode($brand['brand_desc_long']));
    $slug = get_slug($_REQUEST['id'], 'brand');
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    if ((isset($_FILES['brand_logo']['error']) && $_FILES['brand_logo']['error'] == 0) || (!isset($_FILES['brand_logo']['error']) && isset($_FILES['brand_logo']['tmp_name']) && $_FILES['brand_logo']['tmp_name'] != 'none'))
    {
        $logoname = 'logo-'.build_slug($_POST['brand_name']);
        $img_name = basename($image->upload_image($_FILES['brand_logo'],'cdn/brandlogo', $logoname));
        @unlink(ROOT_PATH. CDN_PATH . '/brandlogo/'.$_POST['old_brandlogo']);
    }else{
        $img_name = '';
    }

    $param = "brand_name = '$_POST[brand_name]',  site_url='$site_url', brand_desc='$_POST[brand_desc]', is_show='$is_show', sort_order='$_POST[sort_order]', brand_meta_title='$_POST[brand_meta_title]', brand_keyword='$_POST[brand_keyword]', brand_desc_long='$_POST[brand_desc_long]' ";
    if (!empty($img_name))
    {
        $param .= " ,brand_logo = '$img_name' ";
    }

    update_slug($_POST['id'], 'brand');
...
elseif ($_REQUEST['act'] == 'edit_brand_name')
{
    ...
    else
    {
        ...
        {
            // Update slug
            update_slug_direct($id, $slug, 'brand');
...
elseif($_REQUEST['act'] == 'add_brand')
{
    ...
    else
    {
        ...
        $slug = build_slug($brand);
        create_slug_direct($brand_id, $slug, 'brand');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    if (!empty($logo_name))
    {
        @unlink(ROOT_PATH . CDN_PATH . '/brandlogo/' .$logo_name);
    }

    $exc->drop($id);

    /* Xoá dòng trong bảng catbrand */
    $sql = "DELETE FROM ".$ecs->table('catbrand')." WHERE brand_id = '$id'";
    $db->query($sql);
...
function get_brandlist()
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res))
    {
        $brand_logo = empty($rows['brand_logo']) ? '' :
            '<a href="../' . CDN_PATH . '/brandlogo/'.$rows['brand_logo'].'" target="_brank"><img src="images/picflag.gif" width="16" height="16" border="0" alt='.$GLOBALS['_LANG']['brand_logo'].' /></a>';
