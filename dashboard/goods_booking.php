...
function get_bookinglist()
{
    ...
    $filter['sort_by']    = empty($_REQUEST['sort_by']) ? 'booking_time' : trim($_REQUEST['sort_by']);
    ...
    /* 获取活动数据 */
    $sql = 'SELECT bg.rec_id, bg.sex, bg.tel, bg.link_man, g.goods_id, g.goods_name, bg.goods_number, bg.booking_time, bg.is_dispose '.
    ...
    {
        $row[$key]['goods_url'] = build_uri('goods', array('gid' => $val['goods_id']));
