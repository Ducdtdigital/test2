...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    /* Print template */
    $ispos = isset($_GET['pos']) && $_GET['pos'] == 1 ? true : false;
    if (isset($_GET['print']))
    {
        ...
        $print_layout = $ispos == true ? 'order_print_pos80.html' : 'order_print.html';
        $smarty->display($print_layout);
    }
    /* 打印快递单 */
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
		...
		// if ($_CFG['mobi_user'] != ''  && $_CFG['mobi_password'] != ''  && $_CFG['mobi_user_shipping'] == '1' && $order['mobile'] !='' )
		// {
		// 	send_mobi_info($consignee['mobile'],'user_shipping',$order['order_sn']);
		// }
    }
...
elseif ($_REQUEST['act'] == 'return_list')
{
    ...
    $smarty->assign('return_list',   $result['return']);
    $smarty->assign('filter',       $result['filter']);
    $smarty->assign('record_count', $result['record_count']);
    $smarty->assign('page_count',   $result['page_count']);
    $smarty->assign('sort_update_time', '<img src="images/sort_desc.gif">');
...
elseif ($_REQUEST['act'] == 'return_info')
{
    ...
    if (!empty($rec_id))
    {
      $return=return_list($rec_id);
    }
    else
    {
        die('order does not exist');
    }
   $smarty->assign('return', $return['return'][0]);
    /* 显示模板 */
    $smarty->assign('ur_here', $_LANG['back_operate'] . $_LANG['detail']);
    $smarty->assign('action_link', array('href' => 'order.php?act=return_list', 'text' => '返回列表'));
    assign_query_info();
    $smarty->display('return_info.htm');
    exit; //
}elseif ($_REQUEST['act'] == 'return_act')
{
    /* 检查权限 */
    admin_priv('return_view');

    $rec_id = intval(trim($_POST['rec_id']));
    $return_answer = trim($_POST['return_answer']);
    $return_status = intval(trim($_POST['return_status']));
     ...
     $db->query($sql);
	$links[] = array('text' => '返回列表', 'href' => 'order.php?act=return_list');
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('add_goods' == $step)
    {
        ...
        /* Nobj add Cong them gia thuoc tinh vao */
        $attr_id = isset($_POST['spec_0']) ? $_POST['spec_0'] : 0;
        if($attr_id > 0){
            $goods_price += $db->getOne("SELECT attr_price FROM " . $GLOBALS['ecs']->table('goods_attr') ." WHERE goods_attr_id = $attr_id AND goods_id = $goods_id");
        }
        /* End Nobj add*/
        $goods_attr = '0';
       ...
       $res = $db->query($sql);
        ...
        /* tru ton kho : 1: ngay khi dat hang | 0 khi giao hang */
    ...
    elseif ('goods' == $step)
    {
        if (isset($_POST['next']))
        {
            /* nobj cập nhật chiết khấu bonus ở đấy
            */
            $old_order = order_info($order_id);
            ...
            $order['bonus']         = 0;
            $order['surplus'] = 0;
            $order['order_amount']  = $old_order['goods_amount'] - $order['discount'] -  $old_order['money_paid'];


            if ($order['order_amount'] > 0 && $old_order['user_id'] > 0)
            {
                ...
                {
                  if ($order['integral'] > $user['pay_points']+$old_order['integral'])
                    {
                      sys_msg($_LANG['pay_points_not_enough']);
                    }
            ...
            $order['money_paid']    = $old_order['goods_amount'] - $order['discount'] -  $order['bonus'] -  $order['integral_money'] - $order['surplus'];
    ...
    elseif ('consignee' == $step)
    {
        /* nobi */
        $order = $_POST;
        $order['address'] = empty($order['address']) ? 'Nhận hàng tại shop' : $order['address'];
        ...
        $agency_changed = $old_order['agency_id'] != $order['agency_id'];
          /*
            Tich hop luon ship va payment o day
         */
        $order['shipping_id'] = 3;
        $order['shipping_name'] = addslashes('Giao hàng tại Shop');
        $order['shipping_fee'] = 0;
        $order['insure_fee'] = 0;
        $order['pay_id'] = 1;
        $order['pay_name'] = addslashes('Thanh toán tại shop');
        $order['pay_fee'] = 0;
        ...
        //nobi cap nhat thay doi diem, chiet khau...
        if ($old_order['user_id'] > 0)
        {
            /* 取得用户信息 */
            $user = user_info($old_order['user_id']);
         ...
         /* tru tien vi ao */
        $user_money_change = $old_order['surplus'];
        if ($user_money_change != 0)
        {
         ...
         /* tru diem su dung diem mua hang */
        $pay_points_change = $old_order['integral'];
        ...
        /* Tang diem sau khi mua hang */
        $order_current =  array('order_id' => $order_id, 'extension_code' => '');
        $integral = integral_to_give($order_current);
        log_account_change($user['user_id'], 0, 0, intval($integral['rank_points']), intval($integral['custom_points']), sprintf($_LANG['order_gift_integral'], $old_order['order_sn']));
            //send_order_bonus($order_id);
        /* upadte bonus da su dung */
        if ($order['bonus_id'] > 0)
        {
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    if ($order_id > 0)
    {
        ...
        if (isset($order['shipping_status']) && ($order['shipping_status'] == SS_SHIPPED ||  $order['shipping_status'] == SS_RECEIVED))
        {
            if ($step != 'shipping')
            {
    ...
    elseif ('consignee' == $step)
    {
        ...
        if ($exist_real_goods)
        {
            /* 取得国家 */
            $smarty->assign('country_list', get_regions());
            if (isset($order['country']) && $order['country'] > 0)
            {
                /* 取得省份 */
    ...
    elseif ('shipping' == $step)
    {
        ...
        foreach ($shipping_list AS $key => $shipping)
        {
            ...
            $shipping_list[$key]['free_money'] =  price_format($shipping_configure[1]['value']);
        }
        $smarty->assign('shipping_list', $shipping_list);
    }
...
elseif ($_REQUEST['act'] == 'templates')
{
    ...
    Ueditor('fckeditor', $file_content, '100%', '350px');
...
elseif ($_REQUEST['act'] == 'edit_templates')
{
    ...
    @fwrite($file_name, stripslashes($_POST['fckeditor']));
    @fclose($file_name);
...
elseif ($_REQUEST['act'] == 'operate')
{
    ...
    /* 批量打印订单 */
    elseif (isset($_POST['print']) || isset($_POST['printpos']))
    {
        ...
        foreach ($order_sn_list as $order_sn)
        {
            ...
            $smarty->template_dir = '../' . DATA_DIR;
            $print_layout = isset($_POST['printpos']) ? 'order_print_pos80.html' : 'order_print.html';
            $html .= $smarty->fetch($print_layout) .
        ...
        foreach($order_id_list as $id_order)
        {
            ...
            if($order)
            {
                ...
                if ($order['mobile'] != '')
                {
                    ...
                    $customer_tel = substr($order['mobile'],0,1) != '0' ? "0".$order['mobile'] : $order['mobile'];
                    $phones = array($customer_tel);
                    $content = $_CFG['shop_name'].'. Xác nhận đặt hàng từ '.$order['consignee'].'. Mã đơn hàng: '.$order['order_sn'].', Hotline: '.$_CFG['service_phone'];
                    $sms = new SpeedSMSAPI();
                    $return = $sms->sendSMS($phones, $content, SpeedSMSAPI::SMS_TYPE_CSKH, $_CFG['shop_name']);
                }
...
elseif ($_REQUEST['act'] == 'search_users')
{
    ...
    if ($id_name != '')
    {
        $sql = "SELECT user_id, user_name FROM " . $GLOBALS['ecs']->table('users') .
                ...
                " OR mobile_phone LIKE '%" . mysql_like_quote($id_name) . "%'" . //nobj
                " LIMIT 20";
...
function return_list($rec_id=0)
{
	if($rec_id)$wheres=" o.rec_id = '$rec_id' AND ";
    $result = get_filter();
    if ($result === false)
    {
        $aiax = isset($_GET['is_ajax']) ? $_GET['is_ajax'] : 0;
        ...
        $filter['record_count']   = $GLOBALS['db']->getOne($sql);
        $filter['page_count']     = $filter['record_count'] > 0 ? ceil($filter['record_count'] / $filter['page_size']) : 1;
	   $sql = "SELECT g.goods_thumb,oi.order_sn,g.goods_thumb,g.goods_name,r.return_add_time,o.rec_id,r.return_type,r.return_pic,r.return_desc,r.return_status,r.return_answer, o.goods_id, o.goods_name,o.order_id, o.goods_sn, o.market_price, o.goods_number, " .
				"o.goods_price,oi.consignee,oi.email,oi.shipping_time, o.goods_attr, o.is_real, o.parent_id, o.is_gift, " .
				"o.goods_price * o.goods_number AS subtotal, o.extension_code " .
				"FROM " . $GLOBALS['ecs']->table('order_goods') ." AS o ".
				"LEFT JOIN ". $GLOBALS['ecs']->table('goods') ." AS g ON g.goods_id =o.goods_id ".
				"LEFT JOIN ". $GLOBALS['ecs']->table('return') ." AS r ON r.rec_id =o.rec_id ".
