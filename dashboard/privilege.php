...
{
    /* 清除cookie */
    setcookie('ECSCP[admin_id]',   '', 1, $cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
    setcookie('ECSCP[admin_pass]', '', 1, $cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    if ($row)
    {
        ...
        if (isset($_POST['remember']))
        {
            ...
            setcookie('ECSCP[admin_id]',   $row['user_id'], $time, $cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
            setcookie('ECSCP[admin_pass]', md5($row['password'] . $_CFG['hash_code']), $time, $cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
        }
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('admin_user')." (user_name, email, password, add_time, nav_list, action_list, role_id,fullname,sdt_admin) ".
           "VALUES ('".trim($_POST['user_name'])."', '".trim($_POST['email'])."', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id','".trim($_POST['fullname'])."','".trim($_POST['sdt_admin'])."')";
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, email, password, agency_id, role_id,fullname,sdt_admin FROM " .$ecs->table('admin_user').
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    $admin_sdt  = !empty($_REQUEST['sdt_admin']) ? trim($_REQUEST['sdt_admin']) : '';
    $ec_salt=rand(1,9999);
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    /* 获得当前管理员数据信息 */
    $sql = "SELECT user_id, user_name, email, nav_list, fullname, sdt_admin ".
