...

{
...

    {
        ...
        $sql = "DELETE FROM " . $ecs->table('goods_article2') .
...

    {
        ...
        $urlseo= get_slug($_REQUEST['goods_id'], 'goods');
...

        {
...

            {
                ...
                $db->autoExecute($ecs->table('goods_article2'), $row, 'INSERT');
		...
		$goods_article_list = get_goods_articles2($goods['goods_id']);   // 关联文章
...
/*    create_html_editor('goods_desc', $goods['goods_desc'], 'FCKeditor', 'Normal', '100%', '400');
    ...
    create_html_editor('goods_gift', $goods['goods_gift'], 'goods_gift', 'Basic', '40%', '180');

*/

    CKeditor('goods_desc', html_entity_decode($goods['goods_desc']));

	CKeditor('desc_long', html_entity_decode($goods['desc_long']));
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update'){
    ...
    $goods_name     = isset($_POST['goods_name']) ? filter_var($_POST['goods_name'], FILTER_SANITIZE_STRING) : '';

    $img_name_seo   = empty($title) ? $goods_name : $title;

    $seo_name = build_slug($img_name_seo);
    ...
    $desc_short     =     empty($_POST['desc_short']) ? '' : $_POST['desc_short'];

    $desc_long      =     empty($_POST['desc_long']) ? '' : $_POST['desc_long'];

    $goods_packaged =     empty($_POST['goods_packaged']) ? '' : $_POST['goods_packaged'];

    $real_photo     =     empty($_POST['real_photo']) ? '' : $_POST['real_photo'];

    $goods_thumb_url  =   empty($_POST['goods_thumb_url']) ? '' : $_POST['goods_thumb_url'];

    $is_vat = isset($_POST['is_vat']) ? intval($_POST['is_vat']) : 0;

    $is_service     = isset($_POST['is_service']) ? 1 : 0;

    $is_feature     = isset($_POST['is_feature']) ? 1 : 0;

    $old_ts      =     empty($_POST['old_ts']) ? '' : $_POST['old_ts'];    
...

    {
...

        {
...

            {
                ...
                @unlink(ROOT_PATH.CDN_PATH1.'/'. $row['goods_thumb']);
...

            {
                ...
                @unlink(ROOT_PATH.CDN_PATH1.'/'. $row['goods_img']);
...

        {
            ...
            $goods_thumb = $image->upload_image($_FILES['goods_img']);
        ...
        /* Nếu tạo gallery tự động được bật trong settings */
...

        {
            ...
            $itype = $image->get_filetype($original_img);

            $newname    = dirname($img) . '/' . $seo_name.'-'.gmtime() . $itype;
...

        {
...

            {
                ...
                // tạo ảnh normal chuẩn 400x400 theo ảnh upload lên
                ...
                /* copy ra để tạp gallery img_url và thumb_url*/
...

                {
                    ...
                    $newname = dirname($img) . '/' . $seo_name.'2-'.gmtime() . $itype;
    ...
    /** Nếu như thumb goods được chọn upload thủ công */
    ...
    $shop_price_hcm = !empty($_POST['shop_price_hcm']) ? $_POST['shop_price_hcm'] : 0;

    $shop_price_hn = !empty($_POST['shop_price_hn']) ? $_POST['shop_price_hn'] : 0;
    ...
    $goods_status = isset($_POST['goods_status']) ? intval($_POST['goods_status']) : 1;
...

    {
...

        {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, title, description, goods_name_style, goods_sn, " .

                    "cat_id, brand_id, shop_price, shop_price_hcm, shop_price_hn, market_price, is_promote, promote_price, " .

                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords,goods_cano,goods_sx,goods_schema, goods_brief, " .

                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, goods_status, " .

                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id, ".

                    "online_price, is_vat, is_service, is_feature, desc_short, real_photo, goods_thumb_url, desc_long, goods_packaged, goods_gift, warranty,old_ts) " .

                "VALUES ('$goods_name', '$title', '$description', '$goods_name_style', '$goods_sn', '$catgory_id', " .

                    "'$brand_id', '$shop_price', '$shop_price_hcm', '$shop_price_hn', '$market_price', '$is_promote','$promote_price', ".

                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".

                    "'$_POST[keywords]','$_POST[goods_cano]','$_POST[goods_sx]','$_POST[goods_schema]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".

                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$goods_status', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
...

        {
            ...
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, title, description, goods_name_style, goods_sn, " .
                    ...
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot, is_real, goods_status, " .
                ...
                "VALUES ('$goods_name', '$title', '$description', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$goods_status', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
...

    {
...

        {
            ...
            @unlink(ROOT_PATH.CDN_PATH1.'/'. $row['original_img']);
        ...
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "shop_price_hcm = '$shop_price_hcm', " .

                "shop_price_hn = '$shop_price_hn', " .
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
				...
				"goods_cano = '$_POST[goods_cano]', " .

				"goods_sx = '$_POST[goods_sx]', " .

				"goods_schema = '$_POST[goods_schema]', " .
                ...
                "goods_thumb_url = '$goods_thumb_url', " .

                "desc_long = '$desc_long', " .

                "goods_packaged = '$goods_packaged', " .
...

    {
        ...
        //Nếu có sự thay đổi slug thì mới update

        update_slug($goods_id, 'goods');
...

    {
        ...
        if(isset($_POST['attr_id_list']))
        {
            foreach ($_POST['attr_id_list'] AS $key => $attr_id)
            {
                ...
                $goods_code = $_POST['goods_code_list'][$key];  // Get goods_code from POST data
                if (!empty($attr_value))
                {
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
                        ...
                        $goods_attr_list[$attr_id][$attr_value]['goods_code'] = $goods_code;  // Update goods_code in the array
                    }
                    else
                    {
                        // 如果原来没有，标记为新增
                        ...
                        $goods_attr_list[$attr_id][$attr_value]['attr_price'] = $attr_price;
                        $goods_attr_list[$attr_id][$attr_value]['goods_code'] = $goods_code;  // Insert goods_code in the array
                    }
                }
            }
        }
         ...
         foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price, goods_code)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]', '$info[goods_code]')";
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]', goods_code = '$info[goods_code]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                else
                {
    ...
    /* Thay đổi tên các hình ảnh */



    $original_img = reformat_image_name('goods', $goods_id, $original_img, 'source', $seo_name);

    $goods_img = reformat_image_name('goods', $goods_id, $goods_img, 'goods', $seo_name);

    $goods_thumb = reformat_image_name('goods_thumb', $goods_id, $goods_thumb, 'thumb', $seo_name);
...

    {
...

        {
            ...
            $img = reformat_image_name('gallery', $goods_id, $img, 'source', $seo_name);

            $gallery_img = reformat_image_name('gallery', $goods_id, $gallery_img, '', $seo_name);
        ...
        $gallery_thumb = reformat_image_name('gallery_thumb', $goods_id, $gallery_thumb, 'thumb', $seo_name);
    ...
    handle_gallery_image($goods_id, $_FILES['img_url'], $_POST['img_desc'], $_POST['img_file'], $seo_name);
...

    {
        ...
        @unlink(ROOT_PATH.CDN_PATH1.'/'. $original_img);

        @unlink(ROOT_PATH.CDN_PATH1.'/'. $img);
...
elseif ($_REQUEST['act'] == 'check_goods_code')
{
    check_authz_json('goods_manage');

    $goods_id = intval($_REQUEST['goods_id']);
    $goods_code = htmlspecialchars(json_str_iconv(trim($_REQUEST['goods_code'])));
    
        // Nếu tồn tại, kiểm tra xem nó có trùng với goods_sn trong bảng goods không
        $sql = "SELECT goods_id FROM " . $ecs->table('goods') . " WHERE goods_sn='$goods_code'";
        if ($db->getOne($sql))
        {
            make_json_error($_LANG['goods_sn_exists']);
        }
...

{
...

    {
        ...
        @unlink(ROOT_PATH.CDN_PATH1.'/'. $goods['goods_thumb']);
...

    {
        ...
        @unlink(ROOT_PATH.CDN_PATH1.'/'.$goods['goods_img']);
...

    {
        ...
        @unlink(ROOT_PATH.CDN_PATH1.'/'.$goods['original_img']);
...

    {
...

        {
            ...
            @unlink(ROOT_PATH.CDN_PATH1.'/'.$row['img_url']);
...

        {
            ...
            @unlink(ROOT_PATH.CDN_PATH1.'/'.$row['thumb_url']);
...

        {
            ...
            @unlink(ROOT_PATH.CDN_PATH1.'/'.$row['img_original']);
    ...
    $sql = "DELETE FROM " . $ecs->table('goods_article2') . " WHERE goods_id = '$goods_id'";
...

{
...

    {
        ...
        $sql = "INSERT INTO " . $ecs->table('goods_article2') . " (goods_id, article_id, admin_id) " .
	...
	$arr = get_goods_articles2($goods_id);
...

{
    ...
    $sql = "DELETE FROM " .$ecs->table('goods_article2') . " WHERE " . db_create_in($articles, "article_id") . " AND goods_id = '$goods_id'";
...

{
...

    {
        ...
        $goods_attr_id = handle_goods_attr($product['goods_id'], $id_list, $is_spec_list, $value_price_list, $goods_code_list);
