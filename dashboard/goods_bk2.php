...
*/
    Ueditor('goods_desc', $goods['goods_desc'], '100%', '350px');
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $goods_thumb_url  =   empty($_POST['thumb_url']) ? '' : $_POST['thumb_url'];
    ...
    if (($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') or (($_POST['goods_img_url'] != $_LANG['lab_picture_url'] && $_POST['goods_img_url'] != 'http://') && $is_url_goods_img = 1))
    {
        ...
        if ($_CFG['auto_generate_gallery'])
        {
            ...
            $newname    = dirname($img) . '/' . $name_seo.'-'.gmtime() . $itype;
            if (!copy('../' . $img, '../' . $newname))
            {
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, title, description, goods_name_style, goods_sn, " .
                    ...
                    "online_price, is_vat, is_service, desc_short, real_photo, goods_thumb_url, desc_long, goods_packaged, goods_gift, warranty) " .
