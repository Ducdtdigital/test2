...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if(isset($_POST["sell_note"])) { $sell_note = $_POST['sell_note']; }

    //Code xử lý, insert dữ liệu vào table

        $sql = "UPDATE " .$ecs->table('feedback'). " SET sell_note = '$sell_note' WHERE msg_id = '$_REQUEST[id]'";
        $db->query($sql);
...
if ($_REQUEST['act']=='action_sell')
{
    ...
    /* 更新当前的评论状态为已回复并且可以显示此条评论 */
        $sql = "UPDATE ".$ecs->table('feedback')." SET sell_note = '".$_POST['sell_note']."' WHERE msg_id = '".$_REQUEST['msg_id']."'";
        $db->query($sql);
    /* 邮件通知处理流程 */
    ...
    /* 记录管理员操作 */
    admin_log(addslashes($_LANG['reply']), 'edit', 'users_feedback');

    ecs_header("Location: ?act=view&id=".$_REQUEST['msg_id']."&send_ok=$send_ok\n");
    exit;
}

//xong note của seller
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    else
    {
        /* 提示信息 */
        $link[] = array('text' => $_LANG['back_list'], 'href' => 'user_msg.php?act=list_all');
        sys_msg($_LANG['no_select_feedback'], 0, $link);
    }
}
...
function msg_list()
{
    ...
    $sql = "SELECT f.msg_id, f.user_name, f.msg_title, f.msg_type, f.order_id, f.msg_status, f.msg_time, f.msg_area, f.sell_note, COUNT(r.msg_id) AS reply " .
...
function get_feedback_detail($id)
{
    ...
    $sql = "SELECT T1.*, T2.msg_id AS reply_id, T2.user_name  AS reply_name, u.email AS reply_email, T1.user_tel, ".
