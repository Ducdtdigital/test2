...
function loadPrice(area) {
	...
	// Lấy giá trị city từ radio button
	var city = $('input[name="price_option"]:checked').val();

	$.get('ajax/goods.php', 'act=price&id=' + goodsId + '&attr=' + attr + '&number=' + qty + '&city=' + city, function(response) {
		var res = $.evalJSON(response);
		if (res.err_msg.length > 0) {
			$.fn.colorbox({
				...
				scrolling: !1,
				transition: 'fade',
				html: '<div class="message_box">' + res.err_msg + '<p class="action"><a href="javascript:void(0);" class="button brighter_button" onclick="$.fn.colorbox.close();return !1;"><span>' + lang.confirm + '</span></a></p></div>'
			})
		} else {
...
function buy(id, num, parent) {
	...
	$.post('gio-hang?step=add_to_cart', {
		...
		if (res.error > 0) {
			...
			loadCart('refresh');
			location.href = base_path + 'gio-hang'
		}
	}, 'text')
}
...
function submitSpeSiy(goods_id, num, parent) {
	...
	goods.number = number;
	goods.parent = (typeof(parent) == "undefined") ? 0 : parseInt(parent);
	$.post('ajax/flow.php?step=add_to_cart', {
		goods: $.toJSON(goods)
	}, function(response) {
		var res = $.evalJSON(response);
		if (res.error > 0) {
			...
			if (res.one_step_buy == '1') {
				location.href = base_path + 'gio-hang?step=checkout'
			} else {
	...
	$.post('ajax/flow.php?step=add_to_cart', {
		...
		if (res.error > 0) {
			...
			$.fn.colorbox({
				...
				scrolling: !1,
				transition: 'fade',
				html: '<div class="message_box mb_question">Thêm giỏ hàng thành công!<p class="action"><a href="' + base_path + 'gio-hang" class="button brighter_button"><span>Thanh Toán</span></a><a href="javascript:void(0);" class="tool_link" onclick="$.fn.colorbox.close();return !1;">Mua Tiếp</a></p></div>'
			})
		}
	}, 'text')
}
...
function gotoBuyPage(page, id) {
	$.get('ajax/goods.php?act=gotopage', 'page=' + page + '&id=' + id, function(response) {
		var res = $.evalJSON(response);
