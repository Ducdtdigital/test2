...
class cls_template
{
    ...
    public $cache_dir      = 'temp/caches';
    public $compile_dir    = 'temp/compiled';
    ...
    protected $_echash        = '554fcae493e564ee0dc75bdf2ebf94ca';
    ...
    function display($filename, $cache_id = '')
    {
        ...
        //$out = $this->makeUrlassets($out, $this->baseUrl);
    ...
    private function makeUrlassets($html, $baseurl){
        return preg_replace('#(<link\b.+href=")(?!http)([^"]*)(".*>)#s', '$1'.$baseurl.'$2$3$4', $html);
    }

    private function makeUlrCss($css,$baseulr){
        return preg_replace_callback(
                '/url\s*\(\s*[\\\'"]?(?!(((?:https?:)?\/\/)|(?:data:?:)))([^\\\'")]+)[\\\'"]?\s*\)/is',
                function($m) use ($baseulr) {
                    $link = str_replace('../', '', $m[3]);
                    return 'url("' . $baseulr  . $link.'")';
                },
                $css
            );
    }
    ...
    public function minify_html($input) {
        ...
        return preg_replace(
            array(
                ...
                '#(&nbsp;)&nbsp;(?![<\s])#', // clean up ...
                '#(?<=\>)(&nbsp;)(?=\<)#', // --ibid
                // Remove HTML comment(s) except IE comment(s), BeginRegion, EndRegion
                '#<!--((?!BeginRegion)(?!EndRegion)(?!\\[endif\\])[\\s\\S])*?-->#s'
    ...
    public  function minify_js($input) {
        ...
        return preg_replace(
            array(
                 //remove comments
                '#\s*(?:(?:\/\*(?:[^*]|(?:\*+[^*\/]))*\*+\/)|(?:(?<!\:|\\\|\'|\")\/\/.*))\s*#',
                // Remove white-space(s) around punctuation(s) [^2] Làm
                // Làm Jquery Selector bị lỗi, nên bỏ dấu . đi
                '#\s*([!%&*\(\)\-=+\[\]\{\}|;:,<>?\/])\s*#',
                // Day , ; cuoi cung
                '#[;,]([\]\}])#',
        ...
        {    $tag_sel = explode(' ', $tag);
            ...
            switch ($tag_sel)
            {
                ...
                case 'insert_js':
                    ...
                    return '<?php echo $this->smarty_insert_js(' . $this->make_array($t) . '); ?>';
                    break;
    ...
    /*
    * Dùng hàm này để minify js
    **/
    function smarty_insert_js($args)
    {

        /** Trả về nếu đã tồn tại. Nếu thay đổi file js thì phải xóa app.min.js thủ công để tạo lại */
        $js_name = $this->assets_dir.'/disc/js/'.$args['name'];
        $echojs = '<script  type="text/javascript" src="'.$this->baseUrl.'static/disc/js/'.$args['name'].'"></script>';
        if(file_exists($js_name) && DEBUG_MODE == 0){
            return $echojs;
        }
        /** Còn ko thì đi tạo file */
        static $scriptss = array();
        static $scriptsno = array();
        $minify = explode(',', str_replace(' ', '', $args['minify']));
        $nominify = explode(',', str_replace(' ', '', $args['no_minify']));

        $str = '';
        /* Ko can minify */
        foreach ($nominify AS $no)
        {
            if (in_array($no, $scriptsno) == false)
            {
                $scriptsno[] = $no;
                $file = $this->assets_dir.'/'.$no;
                if(file_exists($file)){
         ...
         /* can minify */
        foreach ($minify AS $val)
        {
            if (in_array($val, $scriptss) == false)
            {
                $scriptss[] = $val;
                $file = $this->assets_dir.'/'.$val;
                if(file_exists($file)){
                    $js = file_get_contents($file);
                    $str .= $this->minify_js($js)."\n";
                }
            }

        }
        file_put_contents($js_name,$str);
        return $echojs;
    }

    /*
    * Dùng hàm này để minify css, rồi đưa thành string vào html
    **/
    function smarty_insert_css($args)
    {
        static $csss = array();
        $arr = explode(',', str_replace(' ', '', $args['files']));
        $is_amp = isset($args['amp']) && $args['amp'] == true ? true : false;
        $str = $is_amp == true ? '<style amp-custom>'."\n" : '<style type="text/css">'."\n";
        foreach ($arr AS $val)
        {
            if (in_array($val, $csss) == false)
            {
                ...
                $file = $this->assets_dir.'/'.$val;
                if(file_exists($file)){
                    $css = $this->minify_css(file_get_contents($file));
                    $css = $this->makeUlrCss($css, $this->baseUrl.'static/');
