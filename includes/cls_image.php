...
class cls_image
{
    ...
    function upload_image($upload, $dir = '', $img_name = '')
    {
        ...
        if ($img_name == '')
        {
            ...
            $img_name = $dir . $img_name.time(). $this->get_filetype($upload['name']);
        }else{
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='', $filename= '')
    {
        ...
        /* nobj */
        $filename = $filename == '' ? $this->unique_name($dir) : $filename;
        ...
        //         $img_format = 'png';
        // }
        /** set mặc định tất cả up lên conver png trừ gif */
        $img_format = $org_info[2] == 'image/gif' ? 'gif' : 'jpg';
        $filename = $filename.'.'.$img_format;
        /* nobj PHPThumb */
        $thumb_options = array('resizeUp' => true, 'jpegQuality' => 100);
        $thumb = PhpThumbFactory::create($img, $thumb_options);//
        $thumb->resize($thumb_width);
        $thumb->cropFromCenter($thumb_width);
...
function make_thumb1($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='', $filename= '')
    {
         $gd = $this->gd_version(); //获取 GD 版本。0 表示没有 GD 库，1 表示 GD 1.x，2 表示 GD 2.x
         if ($gd == 0)
         {
        ...
        $filename = $filename.'.'.$img_format;
        /* nobj PHPThumb */
        $thumb_options = array('resizeUp' => true, 'jpegQuality' => 140);
        $thumb = PhpThumbFactory::create($img, $thumb_options);//
        //$thumb->resize($thumb_width);
       // $thumb->cropFromCenter($thumb_width);
        $thumb->save($dir.$filename, $img_format);
    ...
    function add_watermark($filename, $target_file='', $watermark='', $watermark_place='', $watermark_alpha = 0.65)
    {
        ...
        switch ($source_info[2] )
        {
            ...
            case 'image/jpeg':
            case 2:
                imagejpeg($source_handle, $target, 94);//nobj 相册原图加水印后的图片质量
                break;
