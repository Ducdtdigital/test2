...
function get_cat_child($cat_id, $parent_id){
    ...
    if($parent_id == 0){
        $sql = 'SELECT cat_id, cat_name, class, parent_id, icon ' .
...
function get_sibling_categories($cat_id) {
    // get parent of current category
    $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
    $parent_id = $GLOBALS['db']->getOne($sql);
    
    // if current category is not a top-level category
    if ($parent_id > 0) {
        // get all sibling categories, including the current category
        $sql = 'SELECT cat_id, cat_name, class, parent_id ' .
            'FROM ' . $GLOBALS['ecs']->table('category') .
            "WHERE parent_id = $parent_id AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
        $res = $GLOBALS['db']->getAll($sql);
...
function get_booking($goods_id){
    if($goods_id > 0){
        $sql = 'SELECT sex, link_man, tel, booking_time, is_dispose FROM ' . $GLOBALS['ecs']->table('booking_goods') . " WHERE goods_id = '$goods_id'";
        $row = $GLOBALS['db']->getAll($sql);
        $arr['total'] = count($row);
        foreach ($row AS $key => $val)
        {
            $row[$key]['sex'] = $val['sex'] == 0 ? 'Chị ': 'Anh ';
...
function get_categories_tree($cat_id = 0)
{
    ...
    */
    ...
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name, icon, custom_name, class, parent_id,is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$parent_id' AND is_show = 1 ORDER BY sort_order ASC, cat_id ASC";
        $res = $GLOBALS['db']->getAll($sql);
        foreach ($res AS $row)
        {
            ...
            {
                $cat_arr[$row['cat_id']]['id']   = $row['cat_id'];
                $cat_arr[$row['cat_id']]['name'] = !empty($row['custom_name']) ? $row['custom_name'] : $row['cat_name'];
                $cat_arr[$row['cat_id']]['icon'] = $row['icon'];
                $cat_arr[$row['cat_id']]['class'] = $row['class'];
                $cat_arr[$row['cat_id']]['url']  = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
                $cat_arr[$row['cat_id']]['nav_data'] = get_nav_data($row['cat_id']);
                if (isset($row['cat_id']) != NULL)
                {
...
function get_child_tree($tree_id = 0)
{
    ...
    {
        $child_sql = 'SELECT cat_id, cat_name, icon, custom_name, class, parent_id, is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
               ...
               $three_arr[$row['cat_id']]['name'] = !empty($row['custom_name']) ? $row['custom_name'] : $row['cat_name'];
                $three_arr[$row['cat_id']]['icon'] = $row['icon'];
                $three_arr[$row['cat_id']]['class'] = $row['class'];
               $three_arr[$row['cat_id']]['url']  = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
               $three_arr[$row['cat_id']]['nav_data'] = get_nav_data($row['cat_id']);
               if (isset($row['cat_id']) != NULL)
                   {
...
function get_nav_data($cat_id) {
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('nav') . " WHERE cat_id = '$cat_id' AND ifshow = 1 ORDER BY vieworder ASC";
    $res = $GLOBALS['db']->getAll($sql);
    return $res;
}
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.give_integral, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, g.goods_number, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.desc_short, g.seller_note, g.goods_thumb, g.goods_sthumb, g.goods_thumb_url,g.is_new,g.is_best,g.is_hot, RAND() AS rnd " .
                'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        ...
        $sql .= ' ORDER BY CASE WHEN g.goods_number = 0 THEN 1 ELSE 0 END, g.sort_order, rnd';
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            if($row['give_integral'] == -1){
                $integral = $GLOBALS['_CFG']['integral_scale'] ? round($row['shop_price'] * 100 / $GLOBALS['_CFG']['integral_scale']) : 0;
            }else{
                $integral = $row['give_integral'];
            }
            $goods[$idx]['give_integral']   =  number_format($integral,0,",",".");
            /** End Tinh Diem */
            ...
            $goods[$idx]['seller_note']  = nl2p(strip_tags($row['seller_note']));
            $goods[$idx]['desc_short']   = nl2p(strip_tags($row['desc_short']));
            $goods[$idx]['brand_name']   = isset($goods_data['brand'][$row['goods_id']]) ? $goods_data['brand'][$row['goods_id']] : '';
            $goods[$idx]['goods_style_name']   = add_style($row['goods_name'],$row['goods_name_style']);
            $goods[$idx]['price']    = $row['shop_price'];
            $goods[$idx]['market_price'] = price_format($row['market_price']);
            $goods[$idx]['shop_price']   = price_format($row['shop_price']);
            $goods[$idx]['thumb_url']        = $row['goods_thumb_url'];
            $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb'], true);
            $goods[$idx]['sthumb']       = get_image_path($row['goods_id'], $row['goods_sthumb'], true);
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.seller_note, g.goods_thumb, g.goods_sthumb, b.brand_name, " .
...
/**
 * Lấy danh sách sp ajax new, hot, best Home
 *
 * @access  public
...
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='')
{
    ...
    $sql =  'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'promote_start_date, promote_end_date, g.seller_note, g.desc_short, g.goods_thumb, g.goods_sthumb, b.brand_name ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                    "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods[$idx]['sthumb']       = get_image_path($row['goods_id'], $row['goods_sthumb'], true);
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
        $goods[$idx]['short_style_name'] = $row['goods_name'];
        $idx++;
    }
    return $goods;
}
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, c.parent_id, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
    ...
    if ($row !== false)
    {
        ...
        {
            $promote_price = bargain_price($row['promote_price'], $row['promote_start_date'], $row['promote_end_date']);
            $row['saved_price'] = $row['market_price'] - $promote_price;
            $row['discount'] = get_discount($row['market_price'], $promote_price);
            $row['final_price']  = round($promote_price,0);
            if ($row['is_promote'] == 1 && $row['is_vat'] == 2) {
                $row['promote_price_none_vat'] = price_format(round($promote_price / 1.1));
            }
        }
        else
        {
            $promote_price = 0;
            $row['final_price']  = round($row['shop_price'],0);
            $row['saved_price'] = $row['shop_price'] > 0 ? $row['market_price'] - $row['shop_price']: 0;
            $row['discount'] = $row['shop_price'] > 0 ? get_discount($row['market_price'], $row['shop_price']): 0;
        ...
        if($row['give_integral'] == -1){
            //$integral = $GLOBALS['_CFG']['integral_scale'] ? round($row['shop_price'] * 100 / $GLOBALS['_CFG']['integral_scale']) : 0;
            $integral = $row['shop_price'];
        }else{
            $integral = $row['give_integral'];
        }
        $row['give_integral']   =  number_format($integral,0,",",".");
        /** End Tinh Diem */
        // Get phone information from catphone table
        $sql_phone = "SELECT phone, name, khu_vuc FROM " . $GLOBALS['ecs']->table('catphone') . " WHERE cat_id = '{$row['cat_id']}'";
        $phones = $GLOBALS['db']->getAll($sql_phone);

        if (empty($phones)) {
            // If there is no phone info when cat_id of goods and catphone are the same, 
            // then try to get phone info when cat_id of catphone and parent_id of category are the same
            $sql_phone = "SELECT phone, name, khu_vuc FROM " . $GLOBALS['ecs']->table('catphone') . " WHERE cat_id = '{$row['parent_id']}'";
            $phones = $GLOBALS['db']->getAll($sql_phone);
        }
        ...
        foreach ($phones as $phone) {
            if ($phone['khu_vuc'] == 1) {
                $row['hn'][] = array(
                    'phone' => addslashes($phone['phone']),
                    'name' => addslashes($phone['name'])
                );
            } elseif ($phone['khu_vuc'] == 2) {
                $row['hcm'][] = array(
                    'phone' => addslashes($phone['phone']),
                    'name' => addslashes($phone['name'])
                );
        ...
        if ($row['is_vat'] == 2) {
            $row['shop_price_none_vat'] = price_format(round($row['shop_price'] / 1.1));
        }

        $row['price']               = $row['shop_price'];
		$row['is_vat']               = $row['is_vat'];
        $row['market_price']        = price_format($row['market_price']);
        $row['shop_price_formated'] = price_format($row['shop_price']);
        $row['shop_price_hcm_formated'] = price_format($row['shop_price_hcm']);
        ...
        $row['goods_gift']   = nl2p($row['goods_gift']);
        $row['url']         = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
        $row['brand_goods_url'] = build_uri('brand', array('bid' => $row['brand_id']), $row['goods_brand']);
        /** Cấu hình kỹ thuật */
        if(!empty($row['desc_long'])){
            $cauhinh = explode("\n", str_replace("\r", '', $row['desc_long']));
            $thongso = array();
        ...
        /** Hình ảnh thực tế slide */
        if(!empty($row['real_photo'])){
            $photos = explode("\n", str_replace("\r", '', $row['real_photo']));
            $row['photos'] = $photos;
        }else{
            $row['photos'] = '';
        }
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            $sql = 'SELECT g.goods_id, g.goods_name, g.goods_thumb, g.goods_img, g.shop_price AS org_price, ' .
                    ...
                    "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                        "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
                    "WHERE a.attr_id = '$key' AND g.is_on_sale=1 AND a.attr_value = '$val[value]' AND g.goods_id <> '$GLOBALS[goods_id]' " .
...
{
    /** edit Nobj */
    $sql = 'SELECT `sort_order`, `number` FROM ' . $GLOBALS['ecs']->table('template') .
        " WHERE filename = 'index".$GLOBALS['_device']."' AND type = 1 AND remarks ='' AND id = $cat_id ";
    $res = $GLOBALS['db']->getRow($sql);
    $cat['sort_order'] = $res['sort_order'];
    $num = $res['number'];
    /** end edit Nobj */
    $children = get_children($cat_id);
$sql = 'SELECT g.goods_id, g.give_integral, g.goods_name, g.market_price, g.shop_price AS org_price, g.is_new, g.is_best, g.is_hot, g.goods_number, ' .
        "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
        'g.promote_price, promote_start_date, promote_end_date, g.seller_note, g.desc_short, g.goods_thumb, g.goods_sthumb, b.brand_id,b.brand_logo ' .
    "FROM " . $GLOBALS['ecs']->table('goods') . ' AS g ' .
    "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
            "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
    "LEFT JOIN " . $GLOBALS['ecs']->table('brand') . " AS b ON g.brand_id = b.brand_id " .
    ...
    $order_rule = empty($order_rule) ? 'ORDER BY g.sort_order, CASE WHEN g.goods_number = 0 THEN 1 ELSE 0 END, RAND()' : $order_rule;
    $sql .= $order_rule;
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        {
            $goods[$idx]['promote_price'] = '';$promote_price = 0;
        }
        $comments = getRank($row['goods_id']);
        $goods[$idx]['comment_rank']= round($comments['comment_rank'],1);
        $goods[$idx]['comment_rank_css'] = round($comments['comment_rank']);
        $goods[$idx]['comment_count'] = $comments['comment_count'];
        $goods[$idx]['brand_name'] = $row['brand_name'];
        $goods[$idx]['burl'] = build_uri('brand', array('bid' => $row['brand_id'], 'price_min'=>$price_min, 'price_max'=> $price_max, 'filter_attr'=>$filter_attr_str, 'rc'=>$recommend), $cat['cat_name']);
    
        $goods[$idx]['discount'] = $promote_price > 0 ? get_discount($row['shop_price'], $promote_price): '';
        $goods[$idx]['is_best']      = $row['is_best'];
        $goods[$idx]['is_hot']       = $row['is_hot'];
        $goods[$idx]['goods_number']       = $row['goods_number'];
        $goods[$idx]['is_new']       = $row['is_new'];
        ...
        $goods[$idx]['id']           = $row['goods_id'];
        $goods[$idx]['name']         = $row['goods_name'];
        if (strlen($row['goods_name']) > 63) {
    $goods[$idx]['name_leng'] = $row['goods_name'];
}
        $goods[$idx]['price']        = $row['shop_price'];
        $goods[$idx]['market_price'] = price_format($row['market_price']);
        $goods[$idx]['shop_price']   = price_format($row['shop_price']);
...
function get_cat_brand_list($cat_id){
   $sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num ".
    ...
    foreach ($res AS $key => $val)
    {
        ...
        $brands[$key]['logo'] = 'brandlogo/'.$val['brand_logo'];
...
/**
 * Lấy danh sách danh mục con cho phép hiển thị ra home
 * Nếu mún cho lv3 ra, thì lv2 phải show --> nhược điểm.
 * Mục tiêu: Cứ lấy All show_cat_home = 1 mà có parent_id
 * @param  int $pid
 */
function get_clild_list($pid)
{
    $sql_sub = "SELECT * from ".$GLOBALS['ecs']->table('category')." WHERE parent_id=".$pid." AND is_show = 1 AND show_cat_home = 1 ORDER BY sort_order ASC, cat_id ASC";
...
function get_goods_fittings($goods_list = array())
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if($row['goods_price'] < $row['shop_price']){
            $arr[$temp_index]['discount'] = get_discount($row['shop_price'], $row['goods_price']);
            $arr[$temp_index]['is_discount'] = 1;
        }else{
            $arr[$temp_index]['discount'] = '';
            $arr[$temp_index]['is_discount'] = 0;
        }
