...
function insert_preorder($arr)
{
    ...
    $customers = [];
    if(isset($arr['goods_id']) && $arr['goods_id'] > 0){
        $sql = 'SELECT sex, link_man, tel, booking_time, is_dispose FROM ' . $GLOBALS['ecs']->table('booking_goods') . " WHERE goods_id = '$arr[goods_id]' ORDER BY booking_time DESC";
        $row = $GLOBALS['db']->getAll($sql);
        $customers['total'] = count($row);
        foreach ($row AS $key => $val)
        {
            $row[$key]['sex'] = $val['sex'] == 0 ? 'Chị ': 'Anh ';
            $row[$key]['tel'] =  hidetel($val['tel']);
            $row[$key]['booking_time'] = local_date($GLOBALS['_CFG']['time_format'], $val['booking_time']);
        }
        $customers['list'] = $row;
    }
    $GLOBALS['smarty']->assign('preorder_list', $customers);
    $output = $GLOBALS['smarty']->fetch('library/preorder_list.lbi');
...
function insert_query_info()
{
    ...
    {
        $memory_usage = sprintf($GLOBALS['_LANG']['memory_info'], memory_get_usage() / 104857600);
    }
    else
    {
        $memory_usage = '';
...
function insert_cart_info()
{
   ...
   return sprintf($GLOBALS['_LANG']['cart_info'], $number, price_format($amount, false));
}
...
function insert_ads($arr)
{
    ...
    if (!empty($arr['num']) && $arr['num'] != 1)
    {
        $sql  = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, ' .
                ...
                'ORDER BY ad_sort LIMIT ' . $arr['num'];
    ...
    else
    {
        ...
        {
            $sql  = 'SELECT a.ad_id, a.position_id, a.ad_sort, a.media_type, a.ad_link, a.ad_code, a.ad_name, p.ad_width, '.
                        'p.ad_height, p.position_style, RAND() AS rnd ' .
                    'FROM ' . $GLOBALS['ecs']->table('ad') . ' AS a '.
                    'LEFT JOIN ' . $GLOBALS['ecs']->table('ad_position') . ' AS p ON a.position_id = p.position_id ' .
        ...
        switch ($row['media_type'])
        {
            case 0: // 图片广告
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                        CDN_PATH . "/afficheimg/$row[ad_code]" : $row['ad_code'];
                if(!empty($row["ad_link"])){
                    $ads[] = "<a href='tracking?ad_id=$row[ad_id]'><img src='$src' width='" .$row['ad_width']. "' height='$row[ad_height]' border='0' /></a>";
                }else{
                    $ads[] = "<img src='$src' width='" .$row['ad_width']. "' height='$row[ad_height]' border='0' />";
                }
                $ads_name[] = $row['ad_name'];
                break;
            case 2: // CODE
                $ads[] = $row['ad_code'];
                break;
            case 3: // TEXT
                $ads[] = "<a href='tracking?ad_id=$row[ad_id]' target='_blank'>" .htmlspecialchars($row['ad_code']). '</a>';
                break;
        }
    }
    ...
    $need_cache = $GLOBALS['smarty']->caching;
    $GLOBALS['smarty']->caching = false;
    $GLOBALS['smarty']->assign('ads_name', $ads_name);
