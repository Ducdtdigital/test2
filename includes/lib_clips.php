...
function add_message($message)
{
    ...
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('feedback') .
            " (msg_id, parent_id, user_id, user_name, user_tel, user_email, msg_title, msg_type, msg_status,  msg_content, msg_time, message_img, order_id, msg_area)".
            " VALUES (NULL, 0, '$message[user_id]', '$message[user_name]', '$message[user_tel]', '$message[user_email]', ".
...
function add_booking($booking)
{
    $sql = "INSERT INTO " .$GLOBALS['ecs']->table('booking_goods').
            " VALUES ('', '$_SESSION[user_id]', '$booking[sex]', '$booking[email]', '$booking[linkman]', ".
...
function color_tag(&$tags)
{
    ...
    foreach ($tags AS $key => $val)
    {
        ...
        if ($rewrite)
        {
            ...
            {
                $tags[$key]['url'] = '/tag/' . urlencode(urlencode($val['tag_words']));
            }
            else
            {
                $tags[$key]['url'] = '/tag/' . urlencode($val['tag_words']);
            }
        }
        else
        {
            $tags[$key]['url'] = '/tim-kiem/?keywords='.urlencode($val['tag_words']);
        }
    }
    shuffle($tags);
}
