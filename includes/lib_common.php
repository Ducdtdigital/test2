 ...
 * @return string
 */
function ende($action='', $string='') {
        $output = false;
        $encrypt_method = "AES-256-CBC";

        $secret_key = 'SADFo92jzVnzSj39IUYGvi6eL8v6RvJH';
        $secret_iv = 'bcb04b7e103a0cd8b54763051cef08bf';
        // hash
...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name,  c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.show_cat_home, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
            ...
            foreach($res as $k=>$v)
            {
                ...
                $res[$k]['url'] = build_uri('category', array('cid'=> $v['cat_id']), $v['cat_name']);
            }
            //如果数组过大，不采用静态缓存方式
            if (count($res) <= 1000)
            {
...
function cat_list_menu($re_type = true, $level = 0, $is_show_all = true)
{
    $cat_id = 0; // Sử dụng giá trị mặc định cho cat_id
    static $res = NULL;
    ...
    $options = array_filter($res, function ($option) {
            return $option['parent_id'] == 0;
        });
...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        $arr['youtube']              = !empty($arr['qq']) ? $arr['qq'] : '';
        $arr['fanpage']              = !empty($arr['ww']) ? $arr['ww'] : '';
        $arr['skype']                = !empty($arr['skype']) ? $arr['skype'] : '';
        $arr['zalo']                 = !empty($arr['ym']) ? $arr['ym'] : '';
        $arr['messenger']            = !empty($arr['msn']) ? $arr['msn'] : '';
    ...
    switch ($app)
    {
        case 'category':
            ...
            else
            {
                ...
                $uri = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE id = '$cid' and module='category'");
                if (isset($bid) && $bid!=0)
                {
                    $brand_slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE id = '$bid' and module='brand'");
                    $uri .= '-thuong-hieu-'.$brand_slug;
                }
                $uri .= '.html';
                /** Tách ra có thể tồn tại 1 min | 1 max | cả 2 */
                if (isset($price_min) && $price_min != 0) {
    $pos = strpos($uri, "?");
    $uri .= $pos ? '&amp;' : '?';
    $uri .= 'tu=' . $price_min;
}
if (isset($price_max) && $price_max != 0 && $price_max != PHP_INT_MAX) {
    $pos = strpos($uri, "?");
    $uri .= $pos ? '&amp;' : '?';
    $uri .= 'duoi=' . $price_max;
}
                ...
                if (isset($filter_attr) && !empty($filter_attr) && $filter_attr != '0.0.0.0')
                {
                    $pos =strpos($uri,"?");
            ...
            case 'goods':
                if (empty($gid)) {
                    ...
                    $uri = $GLOBALS['db']->getOne("SELECT slug FROM " . $GLOBALS['ecs']->table('slug') . " WHERE id = '$gid' and module='goods'");
                    $uri .= '.html';
        ...
        case 'brand':
            ...
            {
                //$uri = 'brand-' . $bid;
                $slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE id = '$bid' and module='brand'");
                $uri = 'thuong-hieu-'.$slug.'.html';
                /** Cho cây danh mục ở brand */
                if (isset($cid) && !empty($cid))
                {
                    $cat_slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE id = '$cid' and module='category'");
                    /** Hiển thị cách 1: Đưa về category theo hãng */
                    //$uri = $cat_slug.'.html';
                    /** Hiển thị cách 2: Ở lại brand và lọc theo hãng */
                    $uri = $cat_slug.'/thuong-hieu-'. $slug.'.html';
                }
            ...
            else
            {
                ...
                $slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE id = '$acid' and module='article_cat'");
                $parent_id = $GLOBALS['db']->getOne("SELECT parent_id FROM ".$GLOBALS['ecs']->table('article_cat')." WHERE cat_id = '$acid'");
                $uri = $parent_id > 0 ? 'tin-tuc/'.$slug : $slug;
        ...
        case 'article':
            ...
            {
                $sql ="SELECT s.slug, a.cat_id FROM ".$GLOBALS['ecs']->table('slug')." AS s
                 LEFT JOIN ".$GLOBALS['ecs']->table('article')." AS a ON s.id = a.article_id
                 WHERE id = '$aid' and module='article'";
                $article = $GLOBALS['db']->getRow($sql);
                if($article['cat_id'] < 4){
                    $uri = $article['slug'].'.html';
                }
                else{
                    $uri = $amp == false ? 'tin-tuc/'.$article['slug'].'-'.$aid.'.html' : 'tin-tuc/amp/'.$article['slug'].'-'.$aid.'.html';
                }
            }
        ...
        case 'group_buy':
            ...
            {
                 $goods_slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE module='goods' AND id = (SELECT goods_id FROM " .$GLOBALS['ecs']->table('goods_activity')." WHERE act_id = '$gbid')");
                 $uri = 'nhom-mua/'.$goods_slug.'/'. $gbid;
            }
        ...
        case 'topic':
            ...
            {
                $slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE module= 'topic' AND id = $tid");
                $uri = 'khuyen-mai/'.$slug.'.html';
            }
        ...
        case 'exchange':
                if (empty($sid))
                {
                    ...
                    $uri = 'tich-diem-doi-qua/' . $cid.'/';

                    if (isset($price_min))
                    {
                        $pos=strpos($uri,"?");
                        $uri .= ($pos?'&amp;' : '?') .'integral_min=' . $price_min;
                    }
                    if (isset($price_max))
                    {
                        $pos=strpos($uri,"?");
                        $uri .= ($pos?'&amp;' : '?') .'integral_max=' . $price_max;
                    }
                    if (!empty($page) && $page > 1)
                    {
                        $pos=strpos($uri,"?");
        ...
        case 'exchange_goods':
            ...
            {
                $goods_slug = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('slug')." WHERE module='goods' AND id = (SELECT goods_id FROM " .$GLOBALS['ecs']->table('exchange_goods')." WHERE goods_id = $gid)");
                $uri = 'doi-qua/'.$goods_slug.'/'. $gid;
    ...
    /** Chống XSS attack */
    //$uri = strip_tags($uri);
    $uri = preg_replace("/<script>(.*)<\/script>$/i", "", $uri);
    return $uri;
}
/**
...
function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            ...
            foreach($res as $k=>$v)
            {
                $res[$k]['url'] = build_uri('article_cat', array('acid'=> $v['cat_id']), $v['cat_name']);
            }
...
function get_package_info($id)
{
    ...
    $sql = "SELECT act_id AS id,  act_name AS package_name, act_thumb AS package_img,  goods_id , goods_name, start_time, end_time, act_desc, ext_info".
           " FROM " . $GLOBALS['ecs']->table('goods_activity') .
