 ...
 * @return  array
 */
function assign_ur_here($cat = 0, $str = '', $module='')
{

    $page_title = $GLOBALS['_CFG']['shop_name'];
    $ur_here    = '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                   <a itemprop="item" href="'. $GLOBALS['base_path'] . '"><span itemprop="name">' . $GLOBALS['_LANG']['home'] . '</span></a>
                   <meta itemprop="position" content="1"></li>';

        if (in_array($module, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            if ('category' == $module || 'goods' == $module || 'brand' == $module)
            {
                if ($cat > 0)
                {
            ...
            elseif ('article_cat' == $module || 'article' == $module)
            {
                if ($cat > 0)
                {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $k => $val)
                {
                    ...
                    $position = $start +($total - $k);
                    /** gia tri cuoi cung se chuyen thanh h1 doi voi category */
                    $allow_h1 = ['category', 'article_cat'];
                    if(in_array($module,$allow_h1) && $total+1 == $position){
                         $rname = '<h1 itemprop="name">' . htmlspecialchars($val['cat_name']) . '</h1>';
                    }else{
                        $rname = '<span itemprop="name">' . htmlspecialchars($val['cat_name']) . '</span>';
                    }
                    $ur_here    .= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                       <a itemprop="item" href="'. $GLOBALS['base_path'] . build_uri($type, $args, $val['cat_name']) . '">'.$rname.'</a>
        ...
        elseif ('topic' == $module)
        {
            $topic_name = $GLOBALS['db']->getOne("SELECT title FROM " . $GLOBALS['ecs']->table('topic') ." WHERE topic_id = $cat");
            $page_title  = $topic_name . ' - ' . $page_title;
            $ur_here    .= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
            <a itemprop="item" href="'. $GLOBALS['base_path'] .'khuyen-mai"><span itemprop="name">Khuyến mãi</span></a>
            <meta itemprop="position" content="2"></li>';
            $ur_here    .= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
            <a itemprop="item"><span itemprop="name"><h1>'.$topic_name.'</h1></span></a><meta itemprop="position" content="3"></li>';
        }
    /* breakcrumb for single page  */
    if (!empty($str))
    {
        ...
        if($module == ''){
            $ur_here    .= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                    <a itemprop="item" ><h1 itemprop="name">' . $str. '</h1></a><meta itemprop="position" content="2"></li>';
        }else{
            $ur_here    .= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                    <a itemprop="item" ><span itemprop="name">' . $str. '</span></a><meta itemprop="position" content="2"></li>';
        }
    }
...
/* Trả về thông tin danh mục cha của danh mục sản phẩm hiện tại */
function get_parent_cats_new($cat)
{
    if ($cat == 0)
    {
        return array();
    }
    $arr = $GLOBALS['db']->getRow('SELECT cat_id, cat_name FROM ' . $GLOBALS['ecs']->table('category') .
            " WHERE cat_id = (SELECT parent_id FROM " . $GLOBALS['ecs']->table('category') ." WHERE cat_id = ".$cat." LIMIT 1)");
    return $arr;
}
/**
 * 根据提供的数组编译成页面标题
...
{
    $is_mobile = $GLOBALS['_device'];
    $sql = 'SELECT id, number, type FROM ' . $GLOBALS['ecs']->table('template') .
        " WHERE filename = '$tmp$is_mobile' AND type > 0 AND remarks ='' AND theme='" . $GLOBALS['_CFG']['template'] . "'";
 ...
 * @return  array
 */
function get_shop_help($parent_id = 1)
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'ON a.cat_id = c.cat_id WHERE c.parent_id = '.$parent_id.' AND a.is_open = 1 ' .
...
function get_pager($url, $param, $record_count, $page = 1, $size = 10)
{
    ...
    $pager['record_count'] = $record_count;
    $pager['page_count']   = $page_count;
    $url_seo_page_1 = $url.rtrim($param_url,'&'); /** add by nobj */
    if ($pager['styleid'] == 0)
    {
        $pager['page_first']   = $url_seo_page_1;
    ...
    else
    {
        ...
        $pager['page_first'] = ($page - $_offset > 1 && $_pagenum < $page_count) ? $url_seo_page_1 : '';
        if($page_prev == 1){
            $pager['page_prev']  = ($page > 1) ? $url_seo_page_1 : '';
        }else{
            $pager['page_prev']  = ($page > 1) ? $url_format . $page_prev : '';
        }
        $pager['page_next']  = ($page < $page_count) ? $url_format . $page_next : '';
        ...
        {
            if($i == 1){
                $pager['page_number'][$i] = $url_seo_page_1;
            }else{
                $pager['page_number'][$i] = $url_format . $i;
            }
...
function visit_stats()
{
    ...
    setcookie('ECS[visit_times]', $visit_times, $time + 86400 * 365, $cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
...
function show_message($content, $links = '', $hrefs = '', $type = 'info', $auto_redirect = true)
{
    ...
    $GLOBALS['smarty']->display('message'.$GLOBALS['_ismobile'].'.dwt');
    exit;
}
/**
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('youtube',  $GLOBALS['_CFG']['youtube']);
    $smarty->assign('fanpage',  $GLOBALS['_CFG']['fanpage']);
    $smarty->assign('skype',  $GLOBALS['_CFG']['skype']);
    $smarty->assign('zalo',  $GLOBALS['_CFG']['zalo']);
    $smarty->assign('messenger',  $GLOBALS['_CFG']['messenger']);
    $smarty->assign('map',  $GLOBALS['_CFG']['map']);
    //thông tin sài gòn
    $smarty->assign('service_email_sg', $GLOBALS['_CFG']['service_email_sg']);
    $smarty->assign('service_phone_sg', $GLOBALS['_CFG']['service_phone_sg']);
    $smarty->assign('shop_address_sg',  $GLOBALS['_CFG']['shop_address_sg']);
    $smarty->assign('youtube_sg',  $GLOBALS['_CFG']['youtube_sg']);
    $smarty->assign('fanpage_sg',  $GLOBALS['_CFG']['fanpage_sg']);
    $smarty->assign('zalo_sg',  $GLOBALS['_CFG']['zalo_sg']);
    $smarty->assign('messenger_sg',  $GLOBALS['_CFG']['messenger_sg']); 
    $smarty->assign('map_sg',  $GLOBALS['_CFG']['map_sg']);
    $smarty->assign('lang_sp_best',  $GLOBALS['_CFG']['lang_sp_hot']); 
    $smarty->assign('lang_sp_moi',  $GLOBALS['_CFG']['lang_sp_moi']);
    $smarty->assign('list_email_thb',  $GLOBALS['_CFG']['list_email_thb']);
    // hết thông tin tài gòn
    ...
    $phone_data1 = get_all_phones_by_region();
    $smarty->assign('goods_phone_hn_all', $phone_data1['goods_phone_hn']);
    $smarty->assign('goods_phone_hcm_all', $phone_data1['goods_phone_hcm']);

}
function get_all_phones_by_region()
{
    $result = [];

    // Fetch all phones by region
    foreach ([1 => 'hn', 2 => 'hcm'] as $region_code => $region_name) {
        $sql_phone = "SELECT cp.name, cp.phone, cp.khu_vuc " .
                     "FROM " . $GLOBALS['ecs']->table('phone_sale') . " AS cp " .
                     "WHERE cp.khu_vuc = '$region_code' ";
...
function set_affiliate()
{
    ...
    if (!empty($_GET['u']) && $config['on'] == 1)
    {
        if(!empty($config['config']['expire']))
        {
            ...
            setcookie('ecshop_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * $config['config']['expire'] * $c,$cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
        }
        else
        {
            setcookie('ecshop_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * 24,$cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only); // 过期时间为 1 天
        }
    }
}
/**
...
function get_affiliate()
{
    if (!empty($_COOKIE['ecshop_affiliate_uid']))
    {
        ...
        {
            setcookie('ecshop_affiliate_uid', '', 1,$cookie_path, $cookie_domain, $cookie_secure, $cookie_http_only);
        }
    }
    return 0;
}
    ...
    */
    ...
    if ($GLOBALS['db']->getOne($sql))
    {
        ...
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                ...
                "WHERE a.parent_id = '$parent_id' AND a.is_hide = 1 AND a.cat_type=1 ORDER BY parent_order ASC, a.cat_id ASC, child_order ASC";
    }
    else
    {
        /* 获取当前分类及其父分类 */
        $sql = 'SELECT a.cat_id, a.cat_name, b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order ' .
                'FROM ' . $GLOBALS['ecs']->table('article_cat') . ' AS a ' .
                'LEFT JOIN ' . $GLOBALS['ecs']->table('article_cat') . ' AS b ON b.parent_id = a.cat_id ' .
                "WHERE b.parent_id = '$parent_id' AND b.is_hide = 1 AND b.cat_type = 1 ORDER BY sort_order ASC";
    }
    $res = $GLOBALS['db']->getAll($sql);
    $cat_arr = array();
    foreach ($res AS $row)
...
/**
 * Lấy số lượng LIMIT cài đặt trong template setup
 * @param   string      $template   vd: index
 * @param   string      $library    vd: recommend_best
...
function get_library_number($library, $template = null)
{
    ...
    $num = 0;
    if (isset($lib_list[$template][$library.$GLOBALS['_device']]))
    {
        $num = intval($lib_list[$template][$library.$GLOBALS['_device']]);
    }
    else
    {
      // var_dump('1');
        ...
        $lib = '/library/' . $library.$GLOBALS['_device'] . '.lbi';
