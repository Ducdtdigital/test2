...
include_once(ROOT_PATH . 'includes/facebook-php-sdk-master/src/facebook.php');
...
$not_login_arr =
array('login','act_login','register','act_register','act_edit_password','get_password','send_pwd_email','password', 'signin', 'add_tag', 'collect', 'return_to_cart', 'logout', 'email_list', 'validate_email', 'send_hash_mail', 'order_query', 'is_registered', 'check_email','clear_history','qpassword_name', 'get_passwd_question', 'check_answer', 'loginFacebook','loginGoogle');
...
elseif ($action == 'loginFacebook'){
    include_once(ROOT_PATH . 'includes/lib_passport.php');
    // Retrieve the access token sent via AJAX
    $accessToken = $_POST['accessToken'];
    // Request user data from the Facebook Graph API
    $url = "https://graph.facebook.com/v13.0/me?fields=id,name,email&access_token={$accessToken}";
    ...
    if (isset($responseData['id']) && isset($responseData['name']) && (isset($responseData['email']) || isset($responseData['phone']))) {
        // Extract the username and email from the response
        $email = $responseData['email'];
        $password = "123456";
        if($email == ''){
            $email = $responseData['phone'];
        }
        $username = $email;
        // Check tồn tại tài khoản
        if(exit_register($email) == "0"){
            if (register($username, $password, $email, $other) !== false)
            {
                /*把新注册用户的扩展信息插入数据库*/
                $sql = 'SELECT id FROM ' . $ecs->table('reg_fields') . ' WHERE type = 0 AND display = 1 ORDER BY dis_order, id';   //读出所有自定义扩展字段的id
        ...
        // Handle the error or send an appropriate response back to the AJAX request
        echo 'Failed to retrieve username and email from the access token.';
    }
    
}

elseif ($action == 'loginGoogle'){
    include_once(ROOT_PATH . 'includes/lib_passport.php');
    $accessToken = $_POST['accessToken'];
    ...
    // Check if the response contains the required fields
    if (isset($responseData['name']) && isset($responseData['email'])) {
        // Extract the username and email from the response
        $email = $responseData['email'];
        $password = "123456";
        $username = $email;
...
elseif ($action == 'logout')
{
    if ((!isset($back_act)|| empty($back_act)) && isset($GLOBALS['_SERVER']['HTTP_REFERER']))
    {
        $back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'], 'thanh-vien') ? './' : $GLOBALS['_SERVER']['HTTP_REFERER'];
    }
