...
elseif ($_REQUEST['step'] == 'done')
{
   ...
   /* 判断是不是实体商品 */
    ...
    /* 给商家发邮件 */
    /* 增加是否给客服发送邮件选项 */
    if ($_CFG['send_service_email'] && $_CFG['list_email_thb'] != '')
    {
        ...
        // Tách chuỗi email thành một mảng
        $service_emails = explode(',', $_CFG['list_email_thb']);
        
        // Lặp qua mảng và gửi mail cho từng địa chỉ
        foreach ($service_emails as $service_email) {
            send_mail($_CFG['shop_name'], trim($service_email), $tpl['template_subject'], $content, $tpl['is_html']);
        }
    }
    ...
    if ($_CFG['sms_order_placed'] == '1' && $_CFG['sms_shop_mobile'] != ''){
        //$mobile = '0'.$consignee['mobile'];
        $phones = array($_CFG['sms_shop_mobile']);
        include_once('includes/SpeedSMSAPI.v2.php');
        //$content = "Ma don hang: $order[order_sn]. Cam on quy khach da mua hang tai $_CFG[shop_name]. Hotline: 0903.199.997";
        $content = 'TB: Có đơn hàng số '.$order['order_sn'].' từ '.$order['consignee'].', Tel: 0'.$order['mobile'];
